...
import com.humbhi.blackbox.ui.adapters.OverStopAlertAdapter;
...
public class OverstopAlert extends AppCompatActivity implements RetrofitResponse {


    private RecyclerView rvRecycler;
    ArrayList<AlertVehicleModel> list = new ArrayList<>();
    OverStopAlertAdapter adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_overstop_alert);
    ...
    private void initViews()
    {
        ...
        tvTitle.setText("Overstoppage Alert");
    ...
    private void setAlert(String bbid,String vehicleId,String overspeed)
    {
        ...
        new Retrofit2(this, this, Constants.REQ_SET_IGNITION_ALERT
                ...
                +CommonData.INSTANCE.getCustIdFromDB()+"&allvehicle="+allVehicles+"&overspeedlimit=0&overstoplimit="+overspeed)
                .callServicehitec(true);
    }
            ...
            case Constants.REQ_GET_ALLVEHICLES_ALERTS:
                try
                {
                    ...
                    for (int i = 0; i <data.length() ; i++)
                    {
                        ...
                        model.setOverStopLimit(obj.getString("OverStopLimit"));
                        model.setOverStoppageAlert(obj.getString("OverStopAlert"));
                        model.setMessageType("6");
                        list.add(model);
                    ...
                    {
                        if (list.get(j).getOverStoppageAlert().equals("false"))
                        {
                    ...
                    adapter = new OverStopAlertAdapter(this,list);
                    rvRecycler.setLayoutManager(new LinearLayoutManager(this));
                    rvRecycler.setAdapter(adapter);

                    adapter.onItemSelectedListener(new OverStopAlertAdapter.MyClickListener() {
                        @Override
                        ...
                        public void onSetSpeed(int poss, View view1, EditText et)
                        {
                            messageType = "8";//
                            allVehicles = "false";
                            setSpeedAlert(list.get(poss).getBoxID(),list.get(poss).getFMSVehicleId(),et.getText().toString().trim(),list.get(poss).getOverStoppageAlert());
                        }
