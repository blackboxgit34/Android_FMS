package com.humbhi.blackbox.ui.utils.polylineAnimater;
...
public class MapAnimator {
    ...
    private Polyline overSpeedPolyline;

    private PolylineOptions optionsForeground;

    private PolylineOptions optionsForegroundOS;
...
//        foregroundRouteAnimator.start();
    ...
    public void animateRouteWithMultiplePoly(GoogleMap googleMap, List<LatLng> bangaloreRoute,List<LatLng>overSpeed) {
        Log.e("OverSpeedSize",overSpeed.size()+"");
        if (firstRunAnimSet == null) {
            firstRunAnimSet = new AnimatorSet();
        } else {
            firstRunAnimSet.removeAllListeners();
        ...
        for (int i = 0; i < overSpeed.size(); i++) {
            optionsForegroundOS = new PolylineOptions().add(new LatLng(overSpeed.get(i).latitude,overSpeed.get(i).latitude)).color(Color.RED).width(10);
            overSpeedPolyline = googleMap.addPolyline(optionsForegroundOS);
        }
        ...
        percentageCompletion.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                List<LatLng> foregroundPoints = backgroundPolyline.getPoints();
                List<LatLng> overSpeedPoints = overSpeedPolyline.getPoints();
                ...
                foregroundPolyline.setPoints(foregroundPoints);
                overSpeedPolyline.setPoints(overSpeedPoints);
            }
        });
        percentageCompletion.addListener(new Animator.AnimatorListener() {
            @Override
            ...
            public void onAnimationEnd(Animator animation) {
                foregroundPolyline.setColor(GREY);
                foregroundPolyline.setPoints(backgroundPolyline.getPoints());
                overSpeedPolyline.setColor(Color.RED);
                overSpeedPolyline.setPoints(overSpeedPolyline.getPoints());
            }
        ...
        ValueAnimator colorAnimation = ValueAnimator.ofObject(new ArgbEvaluator(), GREY, Color.BLACK,Color.RED);
        colorAnimation.setInterpolator(new AccelerateInterpolator());
        colorAnimation.setDuration(1200); // milliseconds
        ...
        colorAnimation.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animator) {
                foregroundPolyline.setColor((int) animator.getAnimatedValue());
                overSpeedPolyline.setColor((int) animator.getAnimatedValue());
            }
